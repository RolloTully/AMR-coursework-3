...
error_queue = Queue(2)
time_queue = Queue(20)
ISTE_error_queue = Queue(2)
error_queue.memory = [[0,0,0,0],[0,0,0,0]]#fills the error queue so that a derivative can be computed later
control_gains = np.array([[0.5,0.5,0.5,0.5],
...
def controller(state, target_pos, dt):
    ...
    state = np.array(state[[True,True,True,False,False,True]]).T#Turns the state vector in to a column vector for ease of handeling, we dont care about roll or pitch so we filter them out
