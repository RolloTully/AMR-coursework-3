...
def update_gains(control_gains,ISE_delta, ITAE_delta):
    #Update gains
    P_adjustment = np.array([-0.005 if adj>0 else 0.005 for adj in ISE_delta ])
    D_adjustment = np.array([-0.005 if adj>0 else 0.005 for adj in ITAE_delta ])
...
control_gains = np.array([[0.3,0.3,0.3,0.3],
                          [0.06,0.06,0.06,0.06]]) #Arbiraty starting PD controller gains
review_interval = 12
error_history = []
time_history = []
